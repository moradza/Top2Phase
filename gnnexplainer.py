...
class GNNExplainer:
    ...
    **Arguments**
    ...
    def explain_node(self, x, a, e, node_idx=None, epochs=1000):
        """
        ...
        if self.graph_level:
            self.comp_graph = tf.cast(a, tf.float64)
            self.i = tf.zeros(x.shape[0], dtype=tf.int64)
            self.binary = tf.nn.sigmoid(self.model([x, a, e], training=False))
            self.y_pred = tf.argmax(self.model([x, a, e], training=False), axis=1)
            print(self.y_pred, self.binary)

        self.node_pred = self.y_pred[node_idx]
        self.y_pred = tf.cast(self.y_pred, tf.float64)
        ...
        x_mask = tf.Variable(
            tf.cast(0.5*tf.ones((1,x.shape[1])),tf.float64),
            dtype=tf.float64,
            trainable=True,
            name='x_mask',
        )
        a_mask = tf.Variable(
            tf.cast(tf.random.normal(
                self.comp_graph.shape, stddev=(2.0 / x.shape[0]) ** 0.5
            ),tf.float64),
            dtype=tf.float64,
            trainable=True,
            name='a_mask'
    ...
    def _train_step(self, x, a_mask, x_mask, node_idx):
        with tf.GradientTape() as tape:
            ...
            masked_a = tf.multiply(self.comp_graph, tf.nn.sigmoid(a_mask))
            masked_x = tf.nn.softmax(x * tf.expand_dims(tf.nn.sigmoid(x_mask), -1),axis=-1)

            if self.graph_level:
                pred = self.model([masked_x, masked_a, self.e], training=False)[
                    0, self.node_pred
                ]
    ...
    def _explain_loss_fn(self, y_pred, a_mask, x_mask):
        ...
        # Prediction loss
        pred_loss = -tf.math.log(tf.math.sigmoid(y_pred) + 1e-15) if self.binary < 0.5 else -tf.math.log(1-tf.math.sigmoid(y_pred) + 1e-15)
        # Loss for A
    ...
    def _explainer_cleaning(self, a_mask, x_mask, node_idx, a_thresh):
        ...
        if self.preprocess is not None:
            comp_graph_values = tf.ones_like(a_mask)
...
def k_hop_sparse_subgraph(a, node_idx, k, transformer=None):
    ...
    **Arguments**
    ...
    k_neighs = np.zeros(a.shape[0]).astype("float64").reshape(1, -1)
...
def binary_adj_converter(a_in):
    ...
    **Arguments**
    ...
    a = tf.sparse.SparseTensor(
        a_idx, tf.ones(a_idx.shape[0], dtype=tf.float64), a_in.shape
    )
    return a
