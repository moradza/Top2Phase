...
parser.add_argument('--list_of_graphs', nargs='+', default=['graph_Icehvapor_0.6_1.npz', 'graph_Icehvapor_0.6_18.npz', 'graph_Icehvapor_0.6_30.npz','graph_Icehvapor_0.6_29.npz'], help='list of graphs to use for training save as npz,\
...
parser.add_argument('--dataset_size', type=int,  default=1000*60,  help=' dataset size (int)') #check for dataset <= len(graphs)
parser.add_argument('--learning_rate', type=float, default=0.0001, help=' learning rate')
parser.add_argument('--batch_size', type=int, default=32, help='batch size for each epochs')
...
parser.add_argument('--split_fraction', default=0.1, type=float, help='fraction of data for validation')
parser.add_argument('--save_model_dir', default='./tf_ckpts', help='directory to save the model', required=False)
parser.add_argument('--list_of_phases', default=[0,0,1,1], nargs='+',type=int, help='map graph to phase, start from 0  to n-1',required=False)
...
parser.add_argument('--size_factor', default=3, type=int, help='increase/decrease dimensionality of hidden features layer by layer of GNN/MLP')
parser.add_argument('--kernel_network', default=[8,16, 32], nargs='+',type=int, help='filter generating network size and hidden dimensions graph')
parser.add_argument('--ecc_layers', default=3, type=int, help='number of edge-conditioned convolutional layer')
...
parser.add_argument('--mlp_layers', default=2, type=int, help='number of mlp layers after pooling')
parser.add_argument('--log_name', default='training.log', type=str, help='write losses to this file')
parser.add_argument('--fix_size',nargs='+', default=[4,8,16,8, 1], type=int, help='GNN and MLP hidden size layer by layer')
parser.add_argument('--max_neighs',default=None,type=int, help='max number of oxygens in each graphs')
...
if not args.fix_size is None:
    assert len(args.fix_size) == args.mlp_layers + args.ecc_layers
...
dataset = Top2PhaseDataset(dataset_size, list_of_graphs, rseed=args.random_seed,\
                    data_path=args.data_path, max_neighs=args.max_neighs, permute=False, transforms=NormalizeAdj())
...
S = 4 #dataset.n_edge_features  # Dimension of edge features
